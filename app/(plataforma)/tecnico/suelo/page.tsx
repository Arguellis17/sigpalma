...
async function getAnalisis(fincaId: string | null): Promise<AnalisisSueloListRow[]> {
  ...
  let query = supabase
    .from("analisis_suelo")
    .select(
      "id, ph, humedad_pct, compactacion, fertilidad_completa, textura, aluminio, cic, materia_organica_pct, drenaje_campo, notas, archivo_url, created_at, fecha_analisis, lote_id, finca_id, fincas(nombre), lotes(codigo)"
    )
    .eq("is_voided", false)
