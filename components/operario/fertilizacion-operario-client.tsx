...
export function FertilizacionOperarioClient({
  ...
  function validateForm(): string | null {
    if (!fincaId) return "No tiene finca asignada.";
  ...
  async function onConfirmSubmit() {
    ...
    const res = await registrarAplicacionFertilizacion({
      ...
      metodo_aplicacion: metodo as "manual" | "equipada" | "fertirriego" | "otro",
      justificacion_desviacion: justificacion.trim() || null,
    ...
    <div className="space-y-8">
      {pendientes.length === 0 ? (
        <p className="surface-panel rounded-2xl p-4 text-sm text-muted-foreground">
          ...
          nutricional (HU12) para lotes en producción, podrá registrar la aplicación aquí.
        </p>
      ) : (
        <form
      ...
      <div className="space-y-3">
        <h3 className="text-lg font-semibold tracking-tight">Historial reciente</h3>
        {historial.length === 0 ? (
          <p className="text-sm text-muted-foreground">Sin aplicaciones registradas aún.</p>
        ) : (
          ...
          {selected ? (
            <dl className="space-y-2 text-sm">
              <div className="flex justify-between gap-4">
                ...
                <dd className="font-medium text-right">{selected.insumo_nombre}</dd>
              </div>
              <div className="flex justify-between gap-4">
              ...
              <div className="flex justify-between gap-4">
                ...
                <dd className="font-medium">
                  {cantidad} {labelDosisUnidadPlan(selected.dosis_unidad)}
                </dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-muted-foreground">Dosis programada</dt>
                <dd>
                  {selected.dosis_cantidad}{" "}
                  {labelDosisUnidadPlan(selected.dosis_unidad)}
                </dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-muted-foreground">Método</dt>
                <dd>{METODO_LABEL[metodo] ?? metodo}</dd>
              </div>
            </dl>
          ) : null}
